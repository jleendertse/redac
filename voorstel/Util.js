...
// functie voor het opslaan van een bestand in een S3
exports.putObjectToS3 = function(params, callback) {
  var s3 = new AWS.S3();
  var callbackS3 = function(err, data){
    if (err) {
      console.error(err);
      callback(new Error("Fout bij schrijven in S3: " + err.message)); // fout
    } else {
      console.log('Bestand is opgeslagen in S3 ');
      callback(null);
    }
  };

  s3.putObject(params, callbackS3);
};
